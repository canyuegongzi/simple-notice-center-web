            ...
            el-row(type="flex" justify="center" :gutter="0" v-for="(itemTo, index) in addTaskInfo.to" :key="itemTo.key")
               el-col(:span="12")
                   el-form-item(:label="`推送对象${index + 1}：`"
                       ...
                       el-input(v-model="itemTo.value" placeholder="请输入推送对象" @blur="(e) => emailChange(e, itemTo)")
            ...
            el-row(type="flex" justify="center" :gutter="0")
                el-col(:span="12")
                    el-form-item(label="推送内容：" prop="content")
                        el-input(v-model="addTaskInfo.content" type="textarea" :rows="8" size="mini" placeholder="请输入推送内容")
            el-row(type="flex" justify="center" :gutter="0")
                el-col(:span="12")
                    .footer
...
import {appKeyApi, taskApi, taskConfigApi, taskMessageConfigApi} from "@/api/api";
...
export default class AddTask extends Vue {
  ...
  public addTaskInfoRules = {
    ...
    productAttributeCategoryId: [
      new Rule({message: '类别不能为空', trigger: 'blur'}),
    ],
    inputType: [
      new Rule({message: '属性值的录入方式不能为空', trigger: 'change'}),
    ],
    filterType: [
      new Rule({message: '分类筛选样式不能为空', trigger: 'change'}),
    ],
    timeType: [
    ...
    selectType: [
      new Rule({message: '属性是否可选不能为空', trigger: 'change'}),
    ],
    relatedStatus: [
      new Rule({message: '是否支持手动新增不能为空', trigger: 'change'}),
    ],
    searchType: [
      new Rule({message: '能否进行检索不能为空', trigger: 'change'}),
    ],
    sort: [
  ...
  private token: any;
  private attrId: string | Array<string | null> = '';
  private attrType: string | Array<string | null> = '';
  public goBack()  {
    this.$router.back();
  }
  ...
  public async appChange(val: any) {
      console.log(val);
      const app: any = this.appList.find((item: any) => {
      ...
      const response: any = await $post(taskApi.authApp.url, {
        key: app.keyValue,
        expiresTime: app.expiresTime,
        name: val,
      });
      setTimeout(() => {
        if (response && response === true) {
          this.$message.success('应用鉴权通过');
        } else {
          this.addTaskInfo.app = null;
          this.$message.error('应用鉴权未通过');
        }
        this.loading.close();
      }, 1000);
  }
  ...
  public async getAllConfig1IdList() {
    try {
      const response: any = await $post(taskConfigApi.list.url, {status: 1});
  ...
  public async getAllConfig2IdList() {
    try {
      const response: any = await $post(taskMessageConfigApi.list.url, {status: 1});
  ...
  public async loadDefaultConfigList() {
    ...
    try {
      const response: any = await $post(taskConfigApi.defaultList.url, {type});
  ...
  private async okFun() {
    let params: any;
    const api: any = this.addTaskInfo.taskType === 1 ? taskApi.add : taskApi.messageAdd;
    ...
    this.$refs.form.validate(async (valid: boolean) => {
      ...
      console.log(this.currentConfigList)
      const isExperienceList = this.currentConfigList.filter((item: any) => {
        return item.isExperience === true;
      });
      const canSubmit: any = await $post(taskMessageConfigApi.canSubmit.url, {taskType: this.addTaskInfo.taskType, submitType: isExperienceList.length > 0 ? 2 : 1});
      if (canSubmit.data.flag) {
        const res: any = await $post(api.url, params);
        if (res.success) {
          this.$message({
